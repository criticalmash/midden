...
var templates = {
  ...
  nest: _.template('<div class="midden-nest"><ul class="midden-node">\n<%= children %>\n</ul></div>'),
  longString: _.template('<div class="midden-nest"><div class="midden-node midden-text">\n<%= text %>\n</pre></div>'),
  stack: _.template('<%= path %>:<%= line %>:<%= column %>')
};

var maxLen = 64;

function shortenString(prop){
...
var processValue = function processValue(label, prop){
  ...
  switch(propType){
    case 'String':
      var cleanString = _.escape(prop);
      contents = shortenString(cleanString);
    ...
    case 'Array':
      ...
      break;
    case 'Object':
      length = ', ' + _.keysIn(prop).length + ' properties';
      break;
    default:
      contents = '';
  }
  var templateVars = {
    label:_.isUndefined(label)?'...':_.escape(label),
    type: propType,
    length: length,
    contents: contents
  };
...
var processNested = function processNested(label, prop){
  ...
  if(propType==='Array' || propType==='Object'){
    _(prop).forIn(function(value, key){
    ...
    middenNest = templates.longString({text: _.escape(prop)});
    classes = ' midden-parent';
  }
